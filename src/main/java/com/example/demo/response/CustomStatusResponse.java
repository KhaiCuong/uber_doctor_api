...
public class CustomStatusResponse<T> {
	...
	public ResponseEntity<CustomStatusResponse<?>> METHODNOTALLOWED405(String message) {
		CustomStatusResponse<T> response = new CustomStatusResponse<>(HttpStatus.METHOD_NOT_ALLOWED.value(), message,
				null);
	...
	public ResponseEntity<CustomStatusResponse<?>> INTERNALSERVERERROR500(T data) {
		CustomStatusResponse<T> response = new CustomStatusResponse<>(HttpStatus.INTERNAL_SERVER_ERROR.value(),
				"An error occurred while retrieving the model.", data);
